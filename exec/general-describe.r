...
getDescribe<-function(x) {
  ...
  if (!file.exists(paste0(directory.work,kprefijo,x))) {
    dataset <- fread(paste0(directory.datasets,x),header=TRUE, sep=kcampos_separador) # load file
    dataset[ ,  (kcampos_a_borrar) := NULL    ] 
    dataset<- dataset %>% mutate(clase_binaria = as.factor(ifelse(clase_ternaria == 'CONTINUA',0,1)))
    
    #    d<-describe(dataset)
    d<-summarizeColumns(dataset)
...
files <- dir(path=directory.datasets, pattern="*.txt", full.names=F, recursive=FALSE)
