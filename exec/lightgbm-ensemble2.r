...
for (sem in ksemilla_azar[1:3]) {
  ...
  #aplico el modelo a datos nuevos
  validacion_prediccion  <- stats::predict(  modelo, as.matrix( dataset_validacion_sinclase_sparse) )
  # calculo la ganancia
  gan <-  calcular_ganancia(validacion_prediccion,  dataset_validacion[ , kclase_nomcampo] ) 
  # calculo el AUC
  auc <-  fmetrica_auc_lightgbm( validacion_prediccion,  dataset_validacion[ , kclase_nomcampo] ) 
  ...
  sal.lg[[i]]<-list(modelo=modelo,tiempo=tiempo,ganancia=gan$ganancia,
                    pcorte=gan$prob_corte,
                    probs.apl=aplicacion_prediccion,
                    probs=validacion_prediccion)
  i<-i+1
}
...
for (sem in ksemilla_azar[1:3]) {
  ...
  aplicacion_prediccion  <- predict(  modelo, as.matrix( dataset_aplicacion_sinclase_sparse) )
  #aplico el modelo a datos nuevos
  validacion_prediccion  <- predict(  modelo, as.matrix( dataset_validacion_sinclase_sparse) )
  # calculo la ganancia
  gan <-  calcular_ganancia(validacion_prediccion,  dataset_validacion[ , kclase_nomcampo] ) 
  # calculo el AUC
  auc <-  fmetrica_auc_lightgbm( validacion_prediccion,  dataset_validacion[ , kclase_nomcampo] ) 
  ...
  sal.lg.ant[[i]]<-list(modelo=modelo,tiempo=tiempo,ganancia=gan$ganancia,
                        pcorte=gan$prob_corte,probs.apl=aplicacion_prediccion,probs=validacion_prediccion)
  i<-i+1
}
...
#load("modelos_lgb2016")
#load("modelos_lgb2017")
load("modelos_xg2016")
load("modelos_xg2017")
...
sal.xg[[1]]$probs.apl=predict(sal.xg[[1]]$modelo,dataset_aplicacion_sinclase_sparse)
sal.xg[[2]]$probs.apl=predict(sal.xg[[2]]$modelo,dataset_aplicacion_sinclase_sparse)
sal.xg.ant[[1]]$probs.apl=predict(sal.xg.ant[[1]]$modelo,dataset_aplicacion_sinclase_sparse)
sal.xg.ant[[2]]$probs.apl=predict(sal.xg.ant[[2]]$modelo,dataset_aplicacion_sinclase_sparse)
...
nuevo_febrero<- dataset_aplicacion %>% select(
  ...
  clase_binaria
) %>% mutate(modelo.lg1=sal.lg[[1]]$probs.apl,
             modelo.lg2=sal.lg[[2]]$probs.apl,
             modelo.lg3=sal.lg[[3]]$probs.apl,
             modelo.xg1=sal.xg[[1]]$probs.apl,
             modelo.xg2=sal.xg[[2]]$probs.apl,
             modelo.lg1.ant=sal.lg.ant[[1]]$probs.apl,
             modelo.lg2.ant=sal.lg.ant[[2]]$probs.apl,
             modelo.lg3.ant=sal.lg.ant[[3]]$probs.apl,
             modelo.xg1.ant=sal.xg.ant[[1]]$probs.apl,
             modelo.xg2.ant=sal.xg.ant[[2]]$probs.apl
)
...
saveRDS(nuevo_febrero,file=paste0(directory.datasets,"nuevo_febrero.rds"))
saveRDS(nuevo_abril,file=paste0(directory.datasets,"nuevo_abril.rds"))


febrero_train<-preparar_xgb(nuevo_febrero,kclase_nomcampo)
abril_test<-preparar_matriz(nuevo_abril,kclase_nomcampo)
