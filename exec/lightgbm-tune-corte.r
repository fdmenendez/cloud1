...
args <-  c( "701",  "201712_,201711_,201710_,201709_,201708_,201707_",   "201802_" )
...
fganancia_logistic_lightgbm_binaria2   <- function(probs, clases) 
{
   ...
   vprob_corte_binaria2 <<- val$prob_corte

   return(  list( name = "ganancia", 
                  value =  ifelse(  is.na(val$ganancia) , 0, val$ganancia ) ,
                  higher_better= TRUE 
                )
         )
}
...
#  vprob_corte_binaria2  <<-  x$pprob_corte_binaria2
  ...
  cat( kexperimento,
       ...
       kestimacion,
       karchivo_train, karchivo_test,
       paste( "prob_corte_binaria2=", vprob_corte_binaria2, sep=""),
       "\n", sep="\t", file=karchivo_salida, fill=FALSE, append=TRUE 
     )
...
#  dataset1 <- do.call(rbind, lapply( varchivos1, function(x) fread(x, header=TRUE, sep=kcampos_separador)))
  ...
  dataset1 <- read.datasets(parchivos1,directory.datasets,paste0(kextension,".rds"),"Y","lgb")
