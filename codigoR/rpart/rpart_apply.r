...
#source( "/cloud/cloud1/codigoR/rpart/rpart_apply.r" )
...
faplicar_modelo_parametros  = function( pst_parametros, pdataset_generacion, pdataset_aplicacion, pst_mesgeneracion, pst_mesevaluacion )
{
  ...
  eval( parse( text =  paste("modelo  <- rpart( 
                                                formula,
  ...
  aplicacion_prediccion  <- predict(  modelo, pdataset_aplicacion , type = "prob")


  # calculo la ganancia en los datos de prediccion
  gan <-  fmetrica_ganancia_rpart( aplicacion_prediccion[, kclase_valor_positivo ],  pdataset_aplicacion[ , get(kclase_nomcampo)] )


  # calculo el AUC en testing
  auc <-  fmetrica_auc_rpart( aplicacion_prediccion[, kclase_valor_positivo ],  pdataset_aplicacion[ , get(kclase_nomcampo)] )
...
aplicar_rpart  = function( pst_mesgeneracion, pst_mesevaluacion,  plista_parametros )
{
